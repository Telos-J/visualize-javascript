...
function catchFish() {
    ...
    const body = cat.querySelector("#body");
            gsap.to(head, {rotate: 0, transformOrigin: "60% 100%" });    const tail = cat.querySelector("#tail");
    ...
    tl.to(
        body,
        { rotate: -17.4, x: "-7.5%", y: "17%", transformOrigin: "center" },
        0
    );
    tl.to(tail, { rotate: -16.45, x: "-41%", transformOrigin: "0% 100%" }, 0);
    tl.to(
        frontLegs,
        { rotate: 58.38, x: "5%", y: "27%", transformOrigin: "top" },
        0
    );
